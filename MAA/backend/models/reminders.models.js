...
const addReminder = async (newReminder) => {
    try {
        const result = await client.query(
            'INSERT INTO reminders (user_id, medication_id, dosage, schedule_time, frequency, start_date, end_date) VALUES ($1, $2, $3, $4, $5, $6);', newReminder
        );
...
const updateReminder = async (reminderId, updatedData) => {
    try {
        const query = 'UPDATE reminders SET user_id = $1, medication_id = $2, dosage = $3, schedule_time = $4, frequency = $5, start_date = $6, end_date = $7 WHERE id = $8 RETURNING *;';
        const values = [updatedData.user_id, updatedData.medication_id, updatedData.dosage, updatedData.schedule_time, updatedData.frequency, updatedData.start_date, updatedData.end_date, reminderId];
